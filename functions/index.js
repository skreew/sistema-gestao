...
/**
 * Recalcula o custo total de um produto.
 * @param {object} produtoDoc O documento do produto do Firestore.
 * @return {Promise} Uma promessa que resolve após a atualização do documento.
 */
...
async function recalcularCustoProduto(produtoDoc) {
  ...
  const insumosDocs = await db
    .collection('insumos')
    .where(admin.firestore.FieldPath.documentId(), 'in', insumosIds)
    .get();
  ...
  for (const itemFicha of produtoData.ficha) {
    ...
    if (precoPadrao === undefined) {
      functions.logger.error(
        `Preço para o insumo ${itemFicha.insumo_id} não encontrado!`,
      );
  ...
  functions.logger.log(
    `Produto ${produtoDoc.id} (${produtoData.nome}) atualizado. Novo custo: ${novoCustoTotal.toFixed(2)}`,
  );
...
exports.recalcularCMVaoAtualizarInsumo = functions
  ...
  .onUpdate(async (change, context) => {
    ...
    const produtosQuery = db
      .collection('produtos')
      .where('ficha_insumos_ids', 'array-contains', insumoId);
    ...
    const promises = produtosSnapshot.docs.map((doc) =>
      recalcularCustoProduto(doc),
    );
